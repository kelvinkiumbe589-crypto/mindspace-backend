...
import com.mindspace.service.CampaignService;
...
/**
 * Drives the onboarding/marketing drip. Same shape as {@link ReminderController}:
 * a key-guarded trigger for an external cron, a public one-click unsubscribe, and
 * a logged-in preference toggle for the Settings page.
 */
@RestController
...
public class CampaignController {
    ...
    // Reuses the reminder cron secret so no new env var is needed — point a second
    // cron job at /api/campaigns/run with the same ?key=.
    ...
    // POST /api/campaigns/run?key=SECRET
    @PostMapping("/run")
    public ResponseEntity<Map<String, Object>> run(@RequestParam(required = false) String key) {
        if (triggerKey == null || triggerKey.isBlank()) {
            return ResponseEntity.status(HttpStatus.SERVICE_UNAVAILABLE)
                    .body(Map.of("status", "disabled", "message", "Campaign trigger key is not configured"));
        }
        if (!triggerKey.equals(key)) {
            return ResponseEntity.status(HttpStatus.FORBIDDEN).body(Map.of("status", "forbidden"));
        }
        boolean started = campaignService.triggerAsync();
        return ResponseEntity.accepted().body(Map.of("status", started ? "started" : "already-running"));
    }
    ...
    public ResponseEntity<String> unsubscribe(@RequestParam String token) {
        boolean ok = campaignService.unsubscribe(token);
        String msg = ok
                ? "You've been unsubscribed from MindSpace tips & updates. You can re-enable them anytime in Settings."
                : "This unsubscribe link is invalid or has expired.";
        String html = "<!doctype html><html><head><meta charset=\"utf-8\">" +
                "<meta name=\"viewport\" content=\"width=device-width, initial-scale=1\">" +
                "<title>MindSpace emails</title></head>" +
    ...
    // ── Logged-in preference toggle (Settings page) ──
    @GetMapping("/preference")
    public Map<String, Boolean> getPreference(@AuthenticationPrincipal UserDetails userDetails) {
        return Map.of("enabled", campaignService.getPreference(userDetails.getUsername()));
    }
    ...
    public Map<String, Boolean> setPreference(@AuthenticationPrincipal UserDetails userDetails,
        ...
        campaignService.setPreference(userDetails.getUsername(), enabled);
